// GRACE-X Post Production Module - Film Edition v7.0
(function() {
  'use strict';
  const MODULE_ID = 'post';
  
  window.setupPostBrain = function() {
    if (!window.runModuleBrain) return;
    const outputEl = document.getElementById('post-brain-output');
    const inputEl = document.getElementById('post-brain-input');
    const sendBtn = document.getElementById('post-brain-send');
    ...
    async function handleAsk() {
      ...
      try {
        ...
        console.error('[Post] Brain error:', error);
      }
    }
  ...
  function initModule() {
    setTimeout(() => { if (typeof window.setupPostBrain === 'function') window.setupPostBrain(); }, 500);
  }
  ...
  if (window.GraceXMaster) {
    window.GraceXMaster.registerModule(MODULE_ID, {
      ...
      capabilities: ['ai_assistant', 'edit_versions', 'turnovers', 'deliverables'],
      init: initModule
    });
  }
  
  document.addEventListener('gracex:module:loaded', (e) => { if (e.detail && e.detail.module === MODULE_ID) initModule(); });
  console.log('[Post] Module script loaded');
})();
