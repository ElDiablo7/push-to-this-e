// GRACE-X Casting Module - Film Edition v7.0
(function() {
  'use strict';
  const MODULE_ID = 'casting';
  
  window.setupCastingBrain = function() {
    if (!window.runModuleBrain) return;
    const outputEl = document.getElementById('casting-brain-output');
    const inputEl = document.getElementById('casting-brain-input');
    const sendBtn = document.getElementById('casting-brain-send');
    ...
    async function handleAsk() {
      ...
      try {
        ...
        console.error('[Casting] Brain error:', error);
      }
    }
  ...
  function initModule() {
    setTimeout(() => { if (typeof window.setupCastingBrain === 'function') window.setupCastingBrain(); }, 500);
  }
