// GRACE-X Creative Direction Module - Film Edition v7.0
(function() {
  'use strict';
  const MODULE_ID = 'creative';
  
  window.setupCreativeBrain = function() {
    if (!window.runModuleBrain) return;
    const outputEl = document.getElementById('creative-brain-output');
    const inputEl = document.getElementById('creative-brain-input');
    const sendBtn = document.getElementById('creative-brain-send');
    ...
    async function handleAsk() {
      ...
      try {
        ...
        console.error('[Creative] Brain error:', error);
      }
    }
  ...
  function initModule() {
    setTimeout(() => { if (typeof window.setupCreativeBrain === 'function') window.setupCreativeBrain(); }, 500);
  }
